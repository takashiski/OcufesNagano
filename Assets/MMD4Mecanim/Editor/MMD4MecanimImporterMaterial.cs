...
public partial class MMD4MecanimImporter : ScriptableObject
{
	...
	public class VertexBuilder
	{
		...
		public void Build()
		{
			...
			unchecked {
				...
				float vertexScale = extraData.vertexScale * extraData.importScale;
				...
				streamBuilder.AddInt( (meshCount << 24) | meshVertexCount );
				streamBuilder.AddInt( 2 + meshVertexSDEFValues.Count * 3 );
				streamBuilder.AddInt( meshBoneAttributes.Count + meshVertexAttributes.Count );
				...
				streamBuilder.AddFloat( extraData.importScale );
	...
	public void _CheckFBXMaterial()
	{
		...
		if( skinnedMeshRenderers != null && skinnedMeshRenderers.Length > 0 ) {
			...
			if( !initializeVertex ) {
				if( this.mmdModel != null && this.mmdModel.globalSettings != null ) { // Use for FastCheck.
					if( this.vertexData != null ) {
						float importScaleSub = (float)this.mmdModel.globalSettings.importScale - this.vertexData.importScale;
						float vertexScaleSub = (float)this.mmdModel.globalSettings.vertexScale - this.vertexData.vertexScale;
						if( Mathf.Abs( importScaleSub ) > Mathf.Epsilon ||
							...
							if( extraData != null ) { // Detail check.
								importScaleSub = (float)extraData.importScale - this.vertexData.importScale;
		...
		if( initializeMaterial || initializeIndex || initializeVertex ) {
			...
			if( initializeVertex ) {
				...
				_ProcessVertex( fbxAsset, mmdModel );
	...
	private static void _ProcessVertex( GameObject fbxAsset, MMDModel mmdModel )
	{
		if( fbxAsset == null ) {
			Debug.LogError( "FBXAsset is null." );
			return;
